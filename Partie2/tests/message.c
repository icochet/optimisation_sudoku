...
int main() {
    ...
    clock_t begin = clock();
    candidat(grille2);
    while (!(estGrillePleine(grille2)) && (singletonNu(grille2)|| singletonCache(grille2) || !pairenu(grille2) || pairecache (grille2))) {
    }
    // singletonNu(grille2);
    // singletonCache(grille2);
    // pairenu(grille2);
    // pairecache (grille2);
...
bool pairecache(t_grille1 g) {
    ...
    for (int i = 0; i < TAILLE; i++) {
        for (int j = 0; j < TAILLE; j++) {
            if (g[i][j].nbCandidats > 2) {
                for (int v1 = 0; v1 < g[i][j].nbCandidats - 1; v1++) {
                    for (int v2 = v1 + 1; v2 < g[i][j].nbCandidats; v2++) {
                        
                        // Vérification ligne
                        conteur = 0;
                        for (int k = 0; k < TAILLE; k++) {
                            if (k != j) {
                                for (int cand = 0; cand < g[i][k].nbCandidats; cand++) {
                                    if (g[i][k].candidats[cand] == g[i][j].candidats[v1] || g[i][k].candidats[cand] == g[i][j].candidats[v2]) {
                                        conteur++;
                                        if (conteur > 2) {
                                            break;
                                        }
                        ...
                        if (conteur == 2) {
                            ...
                            for (int supp = 0; supp < g[i][j].nbCandidats; supp++) {
                                if (g[i][j].candidats[supp] != g[i][j].candidats[v1] && g[i][j].candidats[supp] != g[i][j].candidats[v2]) {
                                    retirerCandidat(&g[i][j], g[i][j].candidats[supp]);
                                }
                            }
                            trouve = true;
                            printf("ok (ligne)\n");
                        ...
                        for (int k = 0; k < TAILLE; k++) {
                            if (k != i) {
                                for (int cand = 0; cand < g[k][j].nbCandidats; cand++) {
                                    if (g[k][j].candidats[cand] == g[i][j].candidats[v1] || g[k][j].candidats[cand] == g[i][j].candidats[v2]) {
                                        conteur++;
                                        if (conteur > 2) {
                                            break;
                                        }
                        ...
                        for (int m = coinLigne; m < coinLigne + N; m++) {
                            for (int n = coinColonne; n < coinColonne + N; n++) {
                                if (m != i || n != j) {
                                    for (int cand = 0; cand < g[m][n].nbCandidats; cand++) {
                                        if (g[m][n].candidats[cand] == g[i][j].candidats[v1] || g[m][n].candidats[cand] == g[i][j].candidats[v2]) {
                                            conteur++;
                                            if (conteur > 2) {
                                                break;
                                            }
